...
//  -*- C++ -*-.
#ifndef G4DETECTORS_PHG4FOCALDETECTOR_H
#define G4DETECTORS_PHG4FOCALDETECTOR_H
...
class PHG4FoCalDisplayAction;
class PHG4FoCalSteppingAction;
class PHG4Subsystem;
...
class PHG4FoCalDetector : public PHG4Detector
{
 public:
  //! constructor
  PHG4FoCalDetector(PHG4Subsystem *subsys, PHCompositeNode *Node, const std::string &dnam);

  //! destructor
  virtual ~PHG4FoCalDetector() {}

  //! construct
  virtual void ConstructMe(G4LogicalVolume *world);

  //!@name volume accessors
  int IsInFoCal(G4VPhysicalVolume *) const;
  ...
  void SetUsePreshowerTungsten(bool i = 1) { _preshowertungstenactive = i; }
  void UseMaxLength(const int i = 1) { _usemaxlength = i; }
  void UseFullGeometry(const int i = 1) { _usefullgeom = i; }
  void FillHollowSpace(const int i = 1) { _fillhollow = i; }

  int IsActive() const { return _active; }

  void SuperDetector(const std::string &name) { _superdetector = name; }
  void SetSteppingAction(PHG4FoCalSteppingAction *stpact) { m_SteppingAction = stpact; }
 ...
 private:
  ...
  PHG4FoCalDisplayAction *m_DisplayAction;
  PHG4FoCalSteppingAction *m_SteppingAction;

...
PHG4FoCalSteppingAction::PHG4FoCalSteppingAction(PHG4FoCalDetector* detector, const int absorberactive)
...
PHG4FoCalSteppingAction::~PHG4FoCalSteppingAction()
{
...
//____________________________________________________________________________..
bool PHG4FoCalSteppingAction::UserSteppingAction(const G4Step* aStep, bool)
{
    ...
    if (whichactive > 0)
    {
      if (light_scint_model)
      {
        ...
        if (once && edep > 0)
        {
          ...
          if (Verbosity() > 0)
          {
            cout << "PHG4FoCalSteppingAction::UserSteppingAction::"
                 //
                 << detector_->GetName() << " - "
...
//____________________________________________________________________________..
void PHG4FoCalSteppingAction::SetInterfacePointers(PHCompositeNode* topNode)
{
  ...
  if (!hits_)
  {
    std::cout << "PHG4FoCalSteppingAction::SetTopNode - unable to find " << hitnodename << std::endl;
  }
  if (!absorberhits_)
  {
    if (Verbosity() > 0)
    {
      cout << "PHG4FoCalSteppingAction::SetTopNode - unable to find " << absorbernodename << endl;
    }
  }
}

int PHG4FoCalSteppingAction::FindTowerIndexFromPosition(G4StepPoint* prePoint, int& j, int& k)
{
  ...
  j_0 = (int) ( ( _detector_size + ( prePoint->GetPosition().x() ) ) / _towerdivision ); //TODO focal TOWER SIZE
  k_0 = (int) ( ( _detector_size + ( prePoint->GetPosition().y() ) ) / _towerdivision ); //TODO focal TOWER SIZE
  // FIX THIS TODO ANCHOR
  if(j_0<0){
    cout << "\tj: " << j_0 << " fixed to 0" << endl;
    j_0=0;
  }
  if(k_0<0){
    cout << "\tk: " << k_0 << " fixed to 0" << endl;
    k_0=0;
  }
  // if(prePoint->GetPosition().x()>290) cout << prePoint->GetPosition().x() << "\t" << k_0 << endl;
  j = (j_0 * 1);
  k = (k_0 * 1);
  // cout << "j: " << prePoint->GetPosition().x() << "\t" << _towerdivision << "\t" << _detector_size << "\t" << j_0 << endl;
  // cout << "k: " << prePoint->GetPosition().y() << "\t" << _towerdivision << "\t" << _detector_size << "\t" << k_0 << endl;

  return 0;
}

int PHG4FoCalSteppingAction::FindTowerIndex(G4TouchableHandle touch, int& j, int& k)
{
...
int PHG4FoCalSteppingAction::ParseG4VolumeName(G4VPhysicalVolume* volume, int& j, int& k)
{
